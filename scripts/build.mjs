...
const renderServiceChapters = () => data.services.map((service) => `
  <article class="reveal" id="${escapeHtml(service.id)}">
    ...
    <div class="service-columns"><section><h3>핵심 구축</h3><ul>${service.items.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section><section><h3>고급 기능</h3><ul>${(service.advanced || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section><section><h3>운영과 인프라</h3><ul>${(service.operations || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section><a href="contact/?type=${escapeHtml(service.id)}">이 범위로 문의하기 <i aria-hidden="true">↗</i></a></div>
  </article>`).join("");
